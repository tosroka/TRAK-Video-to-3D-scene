...
def run(VIDEO_PATH, IMAGE_PATH, URL):
    VIDEO_PATH = os.path.normpath(bpy.path.abspath(VIDEO_PATH))
    IMAGE_PATH = os.path.normpath(bpy.path.abspath(IMAGE_PATH))
    ...
    # setup camera
    processing.setup_trak_video_background(VIDEO_PATH)
    ...
    for file_type in ['motion', 'texture']:
        ...
        if file_type=="motion":
            processing.load_motion(save_path, bpy.context.scene.target_object)
        else:
            processing.load_and_assign_texture(save_path, bpy.context.scene.target_object)
